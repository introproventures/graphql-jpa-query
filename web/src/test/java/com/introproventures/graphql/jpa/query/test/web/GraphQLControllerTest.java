...
public class GraphQLControllerTest {
    ...
    private ResultActions perform(final String json) throws Exception {
        return mockmvc.perform(
            post("/graphql").content(json).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)
        );
    }
    ...
    public void testGraphqlGetQueryWithVariables() throws Exception {
        ...
        verify(executor).execute("{Tasks(where: {name: {EQ: \"name\"}}){select{id}}}", args);
    }
    ...
    public void testGraphqlQueryGetUnsupportedMediaType() throws Exception {
        ...
        verify(executor, never()).execute("{Tasks(where: {name: {EQ: \"name\"}}){select{id}}}", null);
    }

    @Test
    public void testGraphqlQueryPostUnsupportedMediaType() throws Exception {
        mockmvc.perform(post("/graphql").contentType(MediaType.TEXT_HTML)).andExpect(status().is(415));
    ...
    public void testGraphqlQuery() throws Exception {
        ...
        verify(executor).execute("{Tasks(where: {name: {EQ: \"name\"}}){select{id}}}", null, null);
    }
    ...
    public void testGraphqlArguments() throws Exception {
        GraphQLQueryRequest query = new GraphQLQueryRequest(
            "query TasksQuery($title: String!){Tasks(where:{name: {EQ: $title}}){select{id name}}}"
        );
        ...
        verify(executor).execute(query.getQuery(), null, variables);
    }
    ...
    public void testGraphqlOperationName() throws Exception {
        ...
        verify(executor).execute(query.getQuery(), operationName, variables);
    }
    ...
    public void testGraphqlArgumentsJson() throws Exception {
        String json =
            "{\"query\": \"{Tasks(where:{name:{EQ: \\\"title\\\"}}){select{ title genre }}\", \"variables\": {\"key\": \"value\"}}";

        ok(json);

        verify(executor)
            .execute("{Tasks(where:{name:{EQ: \"title\"}}){select{ title genre }}", null, singletonMap("key", "value"));
    }
    ...
    public void testGraphqlArgumentsParams() throws Exception {
        ...
        mockmvc
            .perform(post("/graphql").param("query", query).contentType(MediaType.APPLICATION_FORM_URLENCODED))
            .andExpect(status().isOk());
